...
public class MotorOperation
{
    ...
    /**
     * center coordinates of the motor (unit: mm).
     * index 0: x coordinate, 1:y coordinate.
     */
    private double[] m_coordinate_l = new double[2];
    private double[] m_coordinate_r = new double[2];
     ...
     */
    private double cicumference_l;
    private double cicumference_r;

    /**
     * movement distance per 1 step of the motor (unit: mm).
     */
    private double distPer1Step_l;
    private double distPer1Step_r;
    ...
    /**
     * start draw point coordinates (unit: mm).
     * index 0: x coordinate, 1:y coordinate.
     */
    private double[] startCoordinate = new double[2];
    ...
    public MotorOperation(
        ...
        this.cicumference_l = getMotorCircumference(this.m_radius_l);
        this.cicumference_r = getMotorCircumference(this.m_radius_r);

        this.distPer1Step_l = getMotorDistPer1Step(this.m_radius_l, this.m_steps_l);
        this.distPer1Step_r = getMotorDistPer1Step(this.m_radius_r, this.m_steps_r);
        ...
        /**
         * set motor center point coodinate.
         */
        this.m_coordinate_l[0] = initDouble.get("m_coordinate_x_l");
        this.m_coordinate_l[1] = initDouble.get("m_coordinate_y_l");
        this.m_coordinate_r[0] = initDouble.get("m_coordinate_x_r");
        this.m_coordinate_r[1] = initDouble.get("m_coordinate_y_r");

        /**
         * set start draw point coodinate.
         */
        this.startCoordinate[0] = initDouble.get("startCoordinate_x");
        this.startCoordinate[1] = initDouble.get("startCoordinate_y");

        /**
         * set current draw point coodinate.
         */
         ...
         */
        this.currentSteps_l = this.getSteps('l', this.getStartCoordinate(0), this.getStartCoordinate(1));
        this.currentSteps_r = this.getSteps('r', this.getStartCoordinate(0), this.getStartCoordinate(1));
    }
    ...
    private boolean hasInit(Map<String,Integer> initInt, Map<String,Double> initDouble)
    {
        ...
        String [] checkInitDouble = {
            ...
            "m_coordinate_x_l", "m_coordinate_y_l", "m_coordinate_x_r", "m_coordinate_y_r",
    ...
    /**
     * get start draw point coodinate
     * @param index 0:x coodinate / 1:y coodinate
     * @return double
     */
    public double getStartCoordinate(int index)
    {
        if (index == 0 || index == 1) {
            return this.startCoordinate[index];
        }
        return 0.0;
    }
    ...
    /**
     * set current draw point coodinate
     * @param x coodinate (unit: mm)
     * @param y coodinate (unit: mm) 
     */
    ...
    /**
     * get cuurent draw point coodinate
     * @param index 0:x coodinate / 1:y coodinate
     * @return double
     */
    ...
    /**
     * move the draw point.
     * @param to_x: to x coodinate
     * @param to_y: to y coodinate
     */
    public void moveDrawCoodinate(double to_x, double to_y)
    {
        // calculate left motor rotation steps
        int toSteps_l = this.getSteps('l', to_x, to_y);
        ...
        int toSteps_r = this.getSteps('r', to_x, to_y);
        ...
        this.setCurrentCoordinate((toSteps_l * this.distPer1Step_l), (toSteps_r * this.distPer1Step_r));
     ...
     * @param currentCoordinate_x: current draw x coordinate (unit: mm).
     * @param currentCoordinate_y: current draw y coordinate (unit: mm).
     * @return steps
     */
    private int getSteps(char m_position, double currentCoordinate_x, double currentCoordinate_y)
    {
        double[] m_coordinate;
        double cicumference;
        double distPer1Step;
        ...
        switch (m_position) {
            // left motor
            case 'l':
                m_coordinate = this.m_coordinate_l;
                cicumference = this.cicumference_l;
                distPer1Step = this.distPer1Step_l;
            ...
            // right motor
            case 'r':
                m_coordinate = this.m_coordinate_r;
                cicumference = this.cicumference_r;
                distPer1Step = this.distPer1Step_r;
        ...
        double[] contactCoodinate = getContactCoodinate(
            currentCoordinate_x,
            currentCoordinate_y,
            m_coordinate[0],
            pow2Radius,
        ...
        double radian = getAtan2(m_coordinate[0], m_coordinate[1], contactCoodinate[0], contactCoodinate[1]);

        // get the distance from the top of the circle of motor to the draw point
        double distance = getArcDist(cicumference, radian) + getDistance(contactCoodinate[0], contactCoodinate[1], currentCoordinate_x, currentCoordinate_y);

        // convert distance to steps and return
        return convertSteps(distance, distPer1Step);
    }
     ...
     *  index 1: y (unit: mm)
     */
    private static final double[] getContactCoodinate(
        double currentCoordinate_x,
        double currentCoordinate_y,
        ...
        double[] contactCoodinate = new double[2];
        double[] points = new double[4];
        double discriminant, val1, val2;
        double currentCoordinateMinul_x = currentCoordinate_x - m_coordinate_x;
        double pow2_x = Math.pow(currentCoordinateMinul_x, 2);
        double pow2_y = Math.pow(currentCoordinate_y, 2);

        discriminant = (times4Pow4Radius * pow2_x) - (4 * (pow2_x + pow2_y) * (pow4Radius - (pow2Radius * pow2_y)));
        val1 = times2Pow2Radius * currentCoordinateMinul_x;
        val2 = 2 * (pow2_x + pow2_y);

        if (discriminant > 0) {
            points[0] = ((val1 + Math.sqrt(discriminant)) / val2) + m_coordinate_x;
            points[1] = ((val1 - Math.sqrt(discriminant)) / val2) + m_coordinate_x;
        } else if (discriminant == 0) {
            points[0] = (val1 / val2) + m_coordinate_x;
            points[1] = points[0];
        } else {
            points[0] = (val1 / val2) + m_coordinate_x;
            points[1] = (Math.sqrt(discriminant) / val2) + m_coordinate_x;
        }

        points[2] = (pow2Radius - (currentCoordinateMinul_x * (points[0] - m_coordinate_x))) / currentCoordinate_y;

        // adopte the contact coordinates of which the Y coordinate of the contact point is a value what less than or equal to zero
        if (points[2] <= 0) {
            contactCoodinate[0] = points[0];
            contactCoodinate[1] = points[2];
        } else {
            contactCoodinate[0] = points[1];
            contactCoodinate[1] = (pow2Radius - (currentCoordinateMinul_x * (points[1] - m_coordinate_x))) / currentCoordinate_y;
        }

        return contactCoodinate;
    }
     ...
     */
    protected static final String getConsolidatedString(char m_position, String param_name)
    {
     ...
     */
    public static final double getMotorCircumference(double radius)
    {
     ...
     */
    public static final double getMotorDistPer1Step(double radius, double steps)
    {
     ...
     * @param center_x: motor coodinate x (unit: mm).
     * @param center_y: motor coodinate y (unit: mm).
     * @param x: contact coodinate x (unit: mm).
     * @param y: contact coodinate y (unit: mm).
     * @return radian
     */
    private static final double getAtan2(double center_x, double center_y, double x, double y) {
        return Math.atan2(prepareCalcCoodinate(center_y, y), prepareCalcCoodinate(center_x, x));
    }
     ...
     * @param centerCoodinate
     * @param coodinate
     * @return 
     */
    private static final double prepareCalcCoodinate(double centerCoodinate, double coodinate)
    {
        if (centerCoodinate != 0) {
            coodinate -= centerCoodinate;
        }
        if (coodinate < 0) {
            coodinate *= -1;
        }
     ...
     */
    private static final double getArcDist(double circumference, double radian)
    {
     ...
     * @return 
     */
    private static final double getDistance(double from_x, double from_y, double to_x, double to_y)
    {
        double dist_x = (from_x - to_x);
        if (dist_x < 0) {
            dist_x *= -1;
        }
        double dist_y = (from_y - to_y);
        if (dist_y < 0) {
            dist_y *= -1;
        }
     ...
     * @param distPer1Step: moving distance per motor 1 step (unit: mm).
     * @return number of moving steps.
     */
    private static final int convertSteps(double distance, double distPer1Step)
    {
        return (int) Math.round(distance / distPer1Step);
    }
}